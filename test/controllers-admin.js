...
describe('Admin Controllers', () => {
    ...
    it('should load admin dashboard', done => {
        groups.join('administrators', adminUid, err => {
            assert.ifError(err);
            const dashboards = ['/admin', '/admin/dashboard/logins', '/admin/dashboard/users', '/admin/dashboard/topics', '/admin/dashboard/searches'];
            async.each(
                dashboards,
                (url, next) => {
    ...
    describe('admin page privileges', () => {
        ...
        describe('routeMap parsing', () => {
            it('should allow normal user access to admin pages', async function () {
                ...
                const uploadRoutes = ['category/uploadpicture', 'uploadfavicon', 'uploadTouchIcon', 'uploadMaskableIcon', 'uploadlogo', 'uploadOgImage', 'uploadDefaultAvatar'];
        ...
        it('should list all admin privileges', async () => {
            ...
            assert.deepStrictEqual(privs, ['admin:dashboard', 'admin:categories', 'admin:privileges', 'admin:admins-mods', 'admin:users', 'admin:groups', 'admin:tags', 'admin:settings', 'groups:admin:dashboard', 'groups:admin:categories', 'groups:admin:privileges', 'groups:admin:admins-mods', 'groups:admin:users', 'groups:admin:groups', 'groups:admin:tags', 'groups:admin:settings']);
        });
