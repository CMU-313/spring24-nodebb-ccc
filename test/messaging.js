...
describe('Messaging Library', () => {
    ...
    describe('rooms', () => {
        ...
        it('should send a user-leave system message when a user leaves the chat room', done => {
            socketModules.chats.getMessages({ uid: mocks.users.foo.uid }, { uid: mocks.users.foo.uid, roomId: roomId, start: 0 }, (err, messages) => {
                assert.ifError(err);
        ...
        it('should not send a user-leave system message when a user tries to leave a room they are not in', async () => {
            await callv3API('delete', `/chats/${roomId}/users/${mocks.users.baz.uid}`, {}, 'baz');
            const messages = await socketModules.chats.getMessages({ uid: mocks.users.foo.uid }, { uid: mocks.users.foo.uid, roomId: roomId, start: 0 });
            assert.equal(messages.length, 4);
        ...
        it('should send a message to a room', async () => {
            ...
            const raw = await util.promisify(socketModules.chats.getRaw)(socketMock, { mid: messageData.mid });
        ...
        it('should load recent chats of user', done => {
            socketModules.chats.getRecentChats({ uid: mocks.users.foo.uid }, { after: 0, uid: mocks.users.foo.uid }, (err, data) => {
                assert.ifError(err);
        ...
        it('should escape teaser', async () => {
            ...
            const data = await util.promisify(socketModules.chats.getRecentChats)({ uid: mocks.users.foo.uid }, { after: 0, uid: mocks.users.foo.uid });
    ...
    describe('edit/delete', () => {
        ...
        it('should not show deleted message to other users', done => {
            socketModules.chats.getMessages({ uid: mocks.users.herp.uid }, { uid: mocks.users.herp.uid, roomId: roomId, start: 0 }, (err, messages) => {
                assert.ifError(err);
