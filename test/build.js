...
describe('minifier', () => {
    ...
    const scripts = [path.resolve(__dirname, './files/1.js'), path.resolve(__dirname, './files/2.js')].map(script => ({
        srcPath: script,
    ...
    it('.js.bundle() should minify scripts', done => {
        ...
        minifier.js.bundle(
            ...
            err => {
                ...
                assert.strictEqual(fs.readFileSync(destPath).toString(), '(function(n,o){n.doStuff=function(){o.body.innerHTML="Stuff has been done"}})(window,document);function foo(n,o){return\'The person known as "\'+n+\'" is \'+o+" years old"}' + '\n//# sourceMappingURL=minified.js.map');
                done();
            }
        );
    });
    ...
    it('.js.minifyBatch() should minify each script', done => {
        minifier.js.minifyBatch(scripts, false, err => {
            ...
            fs.readFile(scripts[0].destPath, (err, buffer) => {
                assert.ifError(err);
                assert.strictEqual(buffer.toString(), '(function(n,o){n.doStuff=function(){o.body.innerHTML="Stuff has been done"}})(window,document);' + '\n//# sourceMappingURL=1.js.map');
                done();
            });
        });
    });
...
describe('Build', () => {
    ...
    before(done => {
        async.parallel([async.apply(rimraf, path.join(__dirname, '../build/public')), async.apply(db.sortedSetAdd, 'plugins:active', Date.now(), 'nodebb-plugin-markdown')], done);
    });
