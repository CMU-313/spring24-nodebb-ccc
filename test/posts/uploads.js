...
describe('upload methods', () => {
    ...
    describe('.associate()', () => {
        it("should add an image to the post's maintained list of uploads", done => {
            async.waterfall([async.apply(posts.uploads.associate, pid, 'files/whoa.gif'), async.apply(posts.uploads.list, pid)], (err, uploads) => {
                assert.ifError(err);
        ...
        it('should allow arrays to be passed in', done => {
            async.waterfall([async.apply(posts.uploads.associate, pid, ['files/amazeballs.jpg', 'files/wut.txt']), async.apply(posts.uploads.list, pid)], (err, uploads) => {
                assert.ifError(err);
        ...
        it('should not associate a file that does not exist on the local disk', done => {
            async.waterfall([async.apply(posts.uploads.associate, pid, ['files/nonexistant.xls']), async.apply(posts.uploads.list, pid)], (err, uploads) => {
                assert.ifError(err);
    ...
    describe('.dissociate()', () => {
        it("should remove an image from the post's maintained list of uploads", done => {
            async.waterfall([async.apply(posts.uploads.dissociate, pid, 'files/whoa.gif'), async.apply(posts.uploads.list, pid)], (err, uploads) => {
                assert.ifError(err);
                assert.strictEqual(4, uploads.length);
        ...
        it('should allow arrays to be passed in', done => {
            async.waterfall([async.apply(posts.uploads.dissociate, pid, ['files/amazeballs.jpg', 'files/wut.txt']), async.apply(posts.uploads.list, pid)], (err, uploads) => {
                assert.ifError(err);
                assert.strictEqual(2, uploads.length);
