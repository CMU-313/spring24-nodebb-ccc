    ...
    unread.initUnreadTopics = function () {
        ...
        function onNewPost(data) {
            if (data && data.posts && data.posts.length && unreadTopics) {
                ...
                if (parseInt(post.uid, 10) === parseInt(app.user.uid, 10) || (!post.topic.isFollowing && post.categoryWatchState !== watchStates.watching)) {
                    return;
                }

                const tid = post.topic.tid;
                if (!unreadTopics[''][tid] || !unreadTopics.new[tid] || !unreadTopics.watched[tid] || !unreadTopics.unreplied[tid]) {
