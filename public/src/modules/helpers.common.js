...
module.exports = function (utils, Benchpress, relative_path) {
    ...
    function generateChildrenCategories(category) {
        ...
        category.children.forEach(function (child) {
            if (child && !child.isSection) {
                ...
                html += '<span class="category-children-item pull-left">' + '<div role="presentation" class="icon pull-left" style="' + generateCategoryBackground(child) + '">' + '<i class="fa fa-fw ' + child.icon + '"></i>' + '</div>' + '<a href="' + link + '"><small>' + child.name + '</small></a></span>';
            }
        });
    ...
    function spawnPrivilegeStates(member, privileges) {
        ...
        return states
            .map(function (priv) {
                const guestDisabled = ['groups:moderate', 'groups:posts:upvote', 'groups:posts:downvote', 'groups:local:login', 'groups:group:create', 'groups:group:react'];
                ...
                const disabled = (member === 'guests' && (guestDisabled.includes(priv.name) || priv.name.startsWith('groups:admin:'))) || (member === 'spiders' && !spidersEnabled.includes(priv.name)) || (member === 'Global Moderators' && globalModDisabled.includes(priv.name));
    ...
    function buildAvatar(userObj, size, rounded, classNames, component) {
        ...
        const attributes = ['alt="' + userObj.username + '"', 'title="' + userObj.username + '"', 'data-uid="' + userObj.uid + '"', 'loading="lazy"'];
        const styles = [];
