...
module.exports = function (Topics) {
    ...
    Topics.purge = async function (tid, uid) {
        ...
        await Promise.all([db.deleteAll([`tid:${tid}:followers`, `tid:${tid}:ignorers`, `tid:${tid}:posts`, `tid:${tid}:posts:votes`, `tid:${tid}:bookmarks`, `tid:${tid}:posters`]), db.sortedSetsRemove(['topics:tid', 'topics:recent', 'topics:posts', 'topics:views', 'topics:votes', 'topics:scheduled'], tid), deleteTopicFromCategoryAndUser(tid), Topics.deleteTopicTags(tid), Topics.events.purge(tid), Topics.thumbs.deleteAll(tid), reduceCounters(tid)]);
    ...
    async function deleteTopicFromCategoryAndUser(tid) {
        ...
        await Promise.all([db.sortedSetsRemove([`cid:${topicData.cid}:tids`, `cid:${topicData.cid}:tids:pinned`, `cid:${topicData.cid}:tids:posts`, `cid:${topicData.cid}:tids:lastposttime`, `cid:${topicData.cid}:tids:votes`, `cid:${topicData.cid}:tids:views`, `cid:${topicData.cid}:recent_tids`, `cid:${topicData.cid}:uid:${topicData.uid}:tids`, `uid:${topicData.uid}:topics`], tid), user.decrementUserFieldBy(topicData.uid, 'topiccount', 1)]);
    ...
    async function reduceCounters(tid) {
        ...
        await Promise.all([db.incrObjectFieldBy('global', 'postCount', postCountChange), db.incrObjectFieldBy(`category:${topicData.cid}`, 'post_count', postCountChange), db.incrObjectFieldBy(`category:${topicData.cid}`, 'topic_count', incr)]);
    }
};
