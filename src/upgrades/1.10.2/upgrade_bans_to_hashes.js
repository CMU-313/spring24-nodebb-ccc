        ...
        await batch.processSortedSet(
            ...
            async uids => {
                for (const uid of uids) {
                    progress.incr();
                    const [bans, reasons, userData] = await Promise.all([db.getSortedSetRevRangeWithScores(`uid:${uid}:bans`, 0, -1), db.getSortedSetRevRangeWithScores(`banned:${uid}:reasons`, 0, -1), db.getObjectFields(`user:${uid}`, ['banned', 'banned:expire', 'joindate', 'lastposttime', 'lastonline'])]);
